...
R=0.1;
Q = transpose(C)* C; 
Pinf = dare(A,B,Q,R,zeros(3,1),I ); 
Kinf = inv(R+B'*Pinf*B)*B'*Pinf*A;

P = dlyap( (A-B*Kinf)',Q+Kinf'*R*Kinf); 
Q_f = P; 
Q_ = blkdiag(Q,Q,Q,Q,Q_f);
R_ = blkdiag(R,R,R,R);
M = transpose(G) *( Q_) *G  + R_;  
% Apply MPC steps case 2
xVec(:,1) = x0 
yVec(1) = C*x0; 
uVec = []; 
for kk = 1:100
 alpha = G'*Q_*H*xVec(:,kk);
 Usol = quadprog(M,alpha',[],[],[],[],lb,ub);
 uVec(kk) = Usol(1);
 xVec(:,kk+1) = A*xVec(:,kk) + B*uVec(kk);
 yVec(kk+1) = C*xVec(:,kk+1);
...
legend({'R=0.001 & Q=Ct*C','R=0.1 & Q=Ct*C','R=0.001 & Q=I','R=0.1 & Q=I'})
